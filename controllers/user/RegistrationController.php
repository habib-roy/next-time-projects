...
class RegistrationController extends Controller
{
     ...
     * @param array            $config
     */
    public function __construct($id, $module, \dektrium\user\Finder $finder, $config = [])
    {
        $this->finder = $finder;
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    ...
                    ['allow' => true, 'actions' => ['create-user'], 'roles' => ['@']], // https://www.yiiframework.com/doc/guide/2.0/en/security-authorization
                ],
            ],
        ];
    }
    ...
    {
        $this->layout = '@app/views/layouts/adminlte/login';
    ...
    public function actionCreateUser($email, $username, $password)
    {
        /** @var RegistrationForm $model */
        $model = \Yii::createObject(RegistrationForm::className());
        $model->load(['register-form'=>['email'=>$email,'username'=>$username,'password'=>$password]]);
        $model->register();
        return $this->finder->findUser(['username'=>$username])->asarray()->one();
    }
