...
class AdminController extends Controller
{
    ...
    //                 [
    //                     'allow' => true,
    //                     'actions' => ['switch','update-profile-me'],
    //                     'roles' => ['@'],
    //                 ],
    //                 [
    //                     'allow' => true,
    ...
    protected function getNewUser($email=false, $username=false, $password='kebudayaan')
    {
        /** @var User $user */
        $user = \Yii::createObject([
            'class'    => User::className(),
        ...
        $this->trigger(self::EVENT_BEFORE_CREATE, $event);
        $user->load(['User'=>['email'=>$email,'username'=>$username,'password'=>$password]]);
        $user->create();
        $this->trigger(self::EVENT_AFTER_CREATE, $event);
        return $user->id;
    }
    ...
    {
        $string = substr(strtolower($string), 0, 25);
    ...
    public function actionUpdateProfileMe()
    {
        Url::remember('', 'actions-redirect');
        $user = $this->findModel(\Yii::$app->user->getId());
        $user->scenario = 'updateme';
        $event = $this->getUserEvent($user);
        ...
        return $this->render('_accountupdate', [
            'user' => $user,
        ]);
    }
